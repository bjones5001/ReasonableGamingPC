...
// Define a route to fetch the best GPU based on price-performance ratio
app.get('/best-gpu', (req, res) => {
  // Query to fetch the best GPU based on price-performance
  const query = `
    SELECT GPU, RP, Price, PPD
    FROM gpu
    ORDER BY PPD ASC
    LIMIT 1
  `;
...
// Define a route to fetch all GPUs and their comparison with the best GPU
app.get('/gpus', (req, res) => {
  // Query to fetch all GPUs and their comparison with the best GPU
  const query = `
    SELECT GPU, RP, Price, PPD, ((Price - (SELECT Price FROM gpu ORDER BY PPD ASC LIMIT 1)) / (SELECT Price FROM gpu ORDER BY PPD ASC LIMIT 1)) * 100 AS Price_increase, ((RP - (SELECT RP FROM gpu ORDER BY PPD ASC LIMIT 1)) / (SELECT RP FROM gpu ORDER BY PPD ASC LIMIT 1)) * 100 AS Performance_increase
    FROM gpu
    ORDER BY PPD ASC
  `;
